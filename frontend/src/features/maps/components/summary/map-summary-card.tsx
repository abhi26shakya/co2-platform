...
export function MapSummaryCard({ analytics, sourceCount, periodLabel }: Props) {
  ...
  return (
    <div className="glass-strong absolute bottom-4 left-4 z-10 w-72 max-w-[calc(100%-2rem)] rounded-xl overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-150">
      <div className="p-3.5 space-y-2">
        <div className="text-[10px] uppercase tracking-wider text-ground-400">{periodLabel}: My Predictions</div>

        {hasData ? (
          <>
            <div className="flex items-baseline gap-1.5">
              <span className="text-3xl font-bold text-instrument" style={{ fontFamily: "var(--font-display)" }}>
                {formatTonnes(totalEmission)}
              </span>
              <span className="text-sm text-ground-400">t CO₂e</span>
            </div>
            <div className="text-xs text-ground-400 font-mono">
              {sourceCount} facilities · {analytics?.total_predictions ?? 0} predictions
            </div>
          </>
        ) : (
          <p className="text-xs text-ground-500 py-1">
              ...
              {(["monthly", "annual"] as const).map((g) => (
                <button
                  ...
                  className={`px-2 py-0.5 rounded text-[9px] uppercase font-semibold tracking-wide cursor-pointer ${
                    granularity === g ? "bg-sensor/10 text-sensor" : "text-ground-500 hover:text-ground-300"
                  }`}
                >
                  {g}
                </button>
            ...
            <div className="h-14 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  />
                  <Bar dataKey="total_emission" fill="#34d399" radius={[2, 2, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </>
      ...
      {expanded && analytics && (
        <div className="border-t border-ground-800 p-3.5 space-y-1.5 text-[11px] animate-in fade-in duration-150">
          <div className="flex justify-between">
            <span className="text-ground-400">Max single prediction</span>
            <span className="font-mono text-instrument">{analytics.max_emission != null ? `${analytics.max_emission} t/yr` : "—"}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-ground-400">Avg confidence</span>
            <span className="font-mono text-instrument">{analytics.avg_confidence != null ? `${analytics.avg_confidence}%` : "—"}</span>
          </div>
        </div>
      )}
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-1 py-1.5 bg-ground-950/60 text-[10px] font-semibold text-ground-400 hover:text-instrument cursor-pointer border-t border-ground-800"
      >
