...
export default function ReportsPage() {
  ...
  const favoritesCount = reports.filter((r: ReportOut) => r.is_favorite).length;
  const downloadsSum = reports.reduce((s: number, r: ReportOut) => s + (r.downloads_count || 0), 0);
  const sharesSum = reports.reduce((s: number, r: ReportOut) => s + (r.shares_count || 0), 0);
  const storageSum = Math.round(reports.reduce((s: number, r: ReportOut) => s + (r.size_mb || 0), 0) * 10) / 10;
